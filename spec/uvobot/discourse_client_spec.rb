...
RSpec.describe Uvobot::DiscourseClient do
  ...
  describe '.create_topic' do
    it 'returns nil when discourse gem exception is rescued' do
      allow_any_instance_of(DiscourseApi::Client).to receive(:create_topic).and_raise(DiscourseApi::Error, '404')
    ...
    it 'fails loud when different exception class than Discourse::Error is thrown' do
      allow_any_instance_of(DiscourseApi::Client).to receive(:create_topic).and_raise('Whack!')
